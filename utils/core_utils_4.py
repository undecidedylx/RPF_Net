...
class Matrix_Logger(object):
    ...
    def matrix(self,i,str):
        ...
        # plt.show()
        save_path = '/remote-home/sunxinhuan/PycharmProject/data/TGMIL_8_9_返修/results_5_1/Transformer_1×_27_0/matrix'
...
def train(datasets, cur, args):
    ...
    results_dict_val, val_pre_results, val_error, val_auc, acc_logger, val_precision, val_racall, val_F1 = summary(cur, model,val_loader,args.n_classes,str='val')
    print('Val error: {:.4f}, ROC AUC: {:.4f}'.format(val_error, val_auc))

    results_dict_test, test_pre_results, test_error, test_auc, acc_logger, test_precision, test_racall, test_F1 = summary(cur, model, test_loader, args.n_classes, str='test')
...
#                     print('class {} clustering acc {}: correct {}/{}'.format(i, acc, correct, count))
#
#             print('Epoch: {}, train_loss: {:.4f}, train_clustering_loss:  {:.4f}, train_error: {:.4f}'.format(epoch,
#                                                                                                               train_loss,
#                                                                                                               train_inst_loss,
#                                                                                                               train_error))
#             for i in range(n_classes):
...
def train_loop_clam(epoch, model, loader, optimizer, n_classes, bag_weight, writer=None, loss_fn=None):
    ...
    for batch_idx, (data, label) in enumerate(loader):
        ...
        # logits, Y_prob, Y_hat, _, instance_dict = model(data=data, label=label, adj=adj,instance_eval=True,attention_only=False)
        # logits, Y_prob, Y_hat, _, instance_dict = model(data=data, label=label, instance_eval=True)
        logits, Y_prob, Y_hat, _, _ = model(data=data, label=label, instance_eval=False)
