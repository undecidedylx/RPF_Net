...
from models.transGAT_model_1_07 import ADJ
from models.transGAT_model_1_07 import vit_base_patch16_224_in21k as MILFusion
...
def train(datasets, cur, args):
    ...
    save_splits(datasets, ['train', 'val', 'test'], os.path.join(args.results_dir, 'splits_{}.csv'.format(cur))) #保存在results文件中
    print('Done!')
    ...
    #         loss_fn = loss_fn.cuda()
        ...
        # weight = torch.FloatTensor([0.551,0.129, 0.320]).to(device)  # 权重设置，根据统计各类别切片数量
    ...
    if args.model_type in ['clam_sb', 'clam_mb', 'transMIL_mb', 'dsmil', 'GAT', 'transGAT', 'GraphTransformer', 'transMIL']:
        if args.subtyping:
            model_dict.update({'subtyping': True})
        ...
        if args.model_type =='clam_sb':
            ...
            state_dict_weights = torch.load('/remote-home/maojiahui/CLAM/uiss results/TGMIL_SY_GY(ct)/task_SGY_low_vs_high_s1/s_5_checkpoint.pt')
            ...
            # state_dict_weights = torch.load('/remote-home/maojiahui/CLAM/results/TGMIL_SY_GY(wsi lwc2)/task_SGY_low_vs_high_s1/6s_1_checkpoint.pt')
            # model.load_state_dict(state_dict_weights, strict=False)
    ...
    for epoch in range(args.max_epochs):
        if args.model_type in ['clam_sb', 'clam_mb','transMIL_mb','dsmil','transGAT','GraphTransformer', 'transMIL'] and not args.no_inst_cluster:
            train_stop = train_loop_clam(cur, epoch, model, train_loader, optimizer, args.n_classes, args.bag_weight, writer, loss_fn)
    ...
    if args.early_stopping:
        model.load_state_dict(torch.load(os.path.join(args.results_dir, str(early_stopping.counter)+"s_{}_checkpoint.pt".format(cur))))
    else:
...
def train_loop_clam(cur,epoch, model, loader, optimizer, n_classes, bag_weight, writer = None, loss_fn = None):
    ...
    for batch_idx, (data, CT_data, label) in enumerate(loader):
        ...
        # # instance_loss = loss_fn(instance_dict.view(1,-1), label)
        # #
        # # total_loss = 0.5 * loss + 0.5 * instance_loss
        # #
        ...
        if (batch_idx + 1) % 20 == 0:
            print('batch {}, loss: {:.4f}, instance_loss: {:.4f}, weighted_loss: {:.4f}, '.format(batch_idx, loss_value, 0.0, total_loss.item()) +
...
def validate_clam(cur, epoch, model, loader, n_classes, early_stopping = None, writer = None, loss_fn = None, results_dir = None):
    ...
    if early_stopping:
        assert results_dir
        early_stopping(epoch, auc, model, ckpt_name = os.path.join(results_dir, str(early_stopping.counter+1)+"s_{}_checkpoint.pt".format(cur)))
