...
class Matrix_Logger(object):
    ...
    def matrix(self,i,str):
        ...
        # plt.show()
        save_path = '/remote-home/sunxinhuan/PycharmProject/data/TGMIL_8_9_返修/SZG_results/results_0/tranmil_1×_27_2/matrix'
...
def train(datasets, cur, args):
    ...
    if args.model_type in ['clam_sb', 'clam_mb','transMIL_mb','dsmil','GAT','transGAT','transMIL']:
        if args.subtyping:
            model_dict.update({'subtyping': True})
    ...
    for batch_idx, (data, label) in enumerate(loader):
        ...
        if (batch_idx + 1) % 20 == 0:
            print('batch {}, loss: {:.4f}, weighted_loss: {:.4f}, '.format(batch_idx, loss_value, total_loss.item()) +
                  'label: {}, bag_size: {}'.format(label.item(), data.size(1)))
...
def validate_clam(cur, epoch, model, loader, n_classes, early_stopping = None, writer = None, loss_fn = None, results_dir = None):
    ...
    with torch.no_grad():
        for batch_idx, (data, label) in enumerate(loader):
            ...
            data, label = data.to(device), label.to(device)
            results_dict = model(data=data)
            # results_dict = model(data=data, label=label, instance_eval=True)
            Y_hat = results_dict['Y_hat']
            logits = results_dict['logits']
