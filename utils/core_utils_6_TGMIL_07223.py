...
from models.transGAT_model_1_Fusion1 import ADJ
from models.transGAT_model_1_Fusion1 import vit_base_patch16_224_in21k as TransGATMIL
...
def train(datasets, cur, args):
    ...
    for epoch in range(args.max_epochs):
        if args.model_type in ['clam_sb', 'clam_mb','transMIL_mb','dsmil','transGAT','GraphTransformer'] and not args.no_inst_cluster:
    ...
    if args.early_stopping:
        model.load_state_dict(torch.load(os.path.join(args.results_dir, "s_{}_end_checkpoint.pt".format(cur))))
    else:
...
def train_loop_clam(cur,epoch, model, loader, optimizer, n_classes, bag_weight, writer = None, loss_fn = None):
    ...
    for batch_idx, (data, CT_data, label) in tqdm(enumerate(loader)):
        ...
        # logits, Y_prob, Y_hat, _, instance_dict = model(data=data, label=label, instance_eval=True)
...
#         adj = adj.to(device)
#
#         logits, Y_prob, Y_hat, _, _ = model(data=data, label=label, adj=adj, instance_eval=False,
...
#         train_loss += loss_value
#         if (batch_idx + 1) % 20 == 0:
#             print('batch {}, loss: {:.4f}, weighted_loss: {:.4f}, '.format(batch_idx, loss_value, total_loss.item()) +
#                   'label: {}, bag_size: {}'.format(label.item(), data.size(0)))
#
#         error = calculate_error(Y_hat, label)
#         train_error += error
