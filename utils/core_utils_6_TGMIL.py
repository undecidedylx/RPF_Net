...
from models.transGAT_model_1 import ADJ
from models.transGAT_model_1 import vit_base_patch16_224_in21k as TransGATMIL
...
class Matrix_Logger(object):
    ...
    def matrix(self,i,str):
        ...
        # plt.show()
        save_path = '/remote-home/sunxinhuan/PycharmProject/data/zhangmudan/分化/sy_xn/results/TGMIL_123×_sx_0821_2/matrix'
...
class EarlyStopping:
    ...
    def save_checkpoint(self, val_loss, model, ckpt_name):
        ...
        if self.verbose:
            print(f'Validation acc increased ({self.val_auc_max:.6f} --> {val_loss:.6f}).  Saving model ...')
            # print(f'Validation auc increased ({self.val_auc_max:.6f} --> {val_loss:.6f}).  Saving model ...')
        torch.save(model.state_dict(), ckpt_name)
        self.val_auc_max = val_loss
...
def train(datasets, cur, args):
    ...
    if args.bag_loss == 'svm':
        ...
        # weight = torch.FloatTensor([0.551,  0.320,0.129]).to(device)  # 权重设置，根据统计各类别切片数量
        weight = torch.FloatTensor([0.2, 0.2, 0.4, 0.2]).to(device)  # 权重设置，根据统计各类别切片数量
