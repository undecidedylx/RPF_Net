...
def collate_MIL(batch):
	...
	label = [item[1].cpu().detach().numpy().tolist() for item in batch]
	# label = torch.from_numpy(np.array([item[1] for item in batch]))
...
def get_simple_loader(dataset, batch_size=1, num_workers=1):
	...
	loader = DataLoader(dataset, batch_size=1, sampler=SequentialSampler(dataset), **kwargs)
	return loader 
	...
	if not testing:
		if training:
			if weighted:
				...
				loader = DataLoader(split_dataset, batch_size=1, sampler = WeightedRandomSampler(weights, len(weights)), **kwargs)
				...
				loader = DataLoader(split_dataset, batch_size=1, sampler=RandomSampler(split_dataset),**kwargs)
		else:
			# loader = DataLoader(split_dataset, batch_size=1, sampler = SequentialSampler(split_dataset), collate_fn = collate_MIL, **kwargs)
			loader = DataLoader(split_dataset, batch_size=1, sampler=SequentialSampler(split_dataset), **kwargs)
