...
parser.add_argument('--data_root_dir', type=str, default='/remote-home/maojiahui/CLAM/SY_features/',
                    help='data directory')
parser.add_argument('--results_dir', type=str, default='/remote-home/maojiahui/CLAM/necrosis results/WiKG',
...
parser.add_argument('--model_type', type=str, choices=['HECTOR', 'MILFusion', 'WiKG', 'MHIM'], default='WiKG',
...
fold = '4'
# models_dir = '/remote-home/maojiahui/CLAM/uiss results/{}/task_SGY_low_vs_high_s1'.format(args.results_dir.split('/')[-1])
# ckpt_paths = sorted([os.path.join(models_dir, file) for file in os.listdir(models_dir) if file.endswith('.pt') and file.split('_',2)[1] == fold])
ckpt_paths = ['/remote-home/maojiahui/CLAM/necrosis results/{}/task_SGY_low_vs_high_s1/34s_{}_checkpoint.pt'.format(args.results_dir.split('/')[-1],fold)]
...
if __name__ == "__main__":
    ...
    for ckpt_idx in range(len(ckpt_paths)):
        ...
        all_auc.append(auc)
        all_acc.append(acc)
        df.to_csv(os.path.join(args.save_dir, '{}_fold_{}.csv'.format(fold, ckpt_idx)), index=False)
