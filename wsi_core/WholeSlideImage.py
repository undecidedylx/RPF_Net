...
#         self.name = ".".join(path.split("/")[-1].split('.')[:-1])
    ...
    def segmentTissue(self, seg_level=1, sthresh=20, sthresh_up = 255, mthresh=7, close = 0, use_otsu=False,
        ...
        if seg_level > self.wsi.level_count:
    ...
    def visWSI(self, vis_level=0, color = (0,255,0), hole_color = (0,0,255), annot_color=(255,0,0), 
        ...
        # print(vis_level)
        # vis_level = 3
        if vis_level > self.wsi.level_count:
            vis_level = vis_level - 1
    ...
    def visHeatmap(self, scores, coords, vis_level=-1, 
        ...
        if alpha < 1.0:
            ...
            img = self.block_blending(img, top_left, bot_right, vis_level=vis_level,alpha=alpha, blank_canvas=blank_canvas,
                                      block_size= 8)#这里我改动过 从1024*1024 改为512*512
    ...
    def block_blending(self, img, top_left, bot_right,vis_level= -1, alpha=0.5, blank_canvas=False, block_size=1024):
        print('\ncomputing blend')
        downsample = self.level_downsamples[vis_level]
        w = img.shape[1]
        h = img.shape[0]
