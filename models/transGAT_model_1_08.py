...
class MILFusion(nn.Module):
    ...
    def forward(self, **kwargs):
        ...
        # xg = self.fc_kron(xg)
        # ct = self.fc_kron(ct)
        # x_ct = torch.kron(torch.kron(xl, xg), ct)
...
if __name__ == "__main__":
    ...
    data = torch.randn((1, 600, 1024)).cuda()
    ct_data = torch.randn((1, 1, 1024)).cuda()
    adj = torch.randn((600, 600)).cuda()
    ...
    results_dict = model(data=data, CT_data=ct_data, adj=adj)
    print(results_dict)
