...
class Attn_Modality_Gated(nn.Module):
    ...
    def __init__(self, gate_h1, gate_h2, gate_h3, dim1_og, dim2_og, dim3_og, use_bilinear=[True, True, True],
                 scale=[1, 1, 1], p_dropout_fc=0.25):
        ...
        if self.gate_h1:
            ...
            self.linear_z1 = nn.Bilinear(dim1_og, dim2_og + dim3_og, dim1) if self.use_bilinear[0] else nn.Sequential(
                nn.Linear(dim1_og + dim2_og + dim3_og, dim1))
        ...
        if self.gate_h2:
            ...
            self.linear_z2 = nn.Bilinear(dim2_og, dim1_og + dim3_og, dim2) if self.use_bilinear[1] else nn.Sequential(
                nn.Linear(dim1_og + dim2_og + dim3_og, dim2))
        ...
        if self.gate_h3:
            ...
            self.linear_z3 = nn.Bilinear(dim3_og, dim1_og + dim2_og, dim3) if self.use_bilinear[2] else nn.Sequential(
                nn.Linear(dim1_og + dim2_og + dim3_og, dim3))
    ...
    def forward(self, x1, x2, x3):
        ...
        if self.gate_h1:
            ...
            z1 = self.linear_z1(x1, torch.cat([x2, x3], dim=-1)) if self.use_bilinear[0] else self.linear_z1(
                torch.cat((x1, x2, x3), dim=-1))  # creates a vector combining both modalities
        ...
        if self.gate_h2:
            ...
            z2 = self.linear_z2(x2, torch.cat([x1, x3], dim=-1)) if self.use_bilinear[1] else self.linear_z2(
        ...
        if self.gate_h3:
            ...
            z3 = self.linear_z3(x3, torch.cat([x1, x2], dim=-1)) if self.use_bilinear[2] else self.linear_z3(
                torch.cat((x1, x2, x3), dim=-1))
...
class HECTOR(nn.Module):
    def __init__(
        ...
        self.post_compression_layer_he = FC_block(dim_post_compression, dim_post_compression // 16,
                                                  p_dropout_fc=p_dropout_fc)
        dim_post_compression = dim_post_compression // 2

        # Post-compression layer.
        dim1, dim2, dim3 = dim_post_compression, self.out_stage_size // scale[
            1] if self.gate_stage else self.out_stage_size, h_mol_size_out // scale[
            2] if self.gate_mol else h_mol_size_out
    ...
    def forward(self, data, CT_data):
        ...
        CT_data = torch.squeeze(CT_data, dim=0)
        if data.shape[0] >= 6000:
            data = data[:6000,:]

        # H&E embedding.
        h = self.compression_layer(data)
        stage = self.compression_layer(CT_data)
        ...
        # stage = self.encoding_stage_net(CT_data)

        # Compression h_mol.
        # h_mol = self.encoding_mol_net(h_mol)

        # Attention gates on each modality.
        h_hist, stage, h_mol = self.attn_modalities(h_hist, stage, stage)

        # Post-compressiong H&E slide embedding.
        h_hist = self.post_compression_layer_he(h_hist)
        stage = self.post_compression_layer_he(stage)

        # Fusion.
        m = self.forward_fusion(h_hist, stage, stage)
...
if __name__ == '__main__':
    h = torch.randn((6000,1024)).cuda()
    h1 = torch.randn((6000,1024)).cuda()
    h2 = torch.randn((6000,1024)).cuda()
    model = HECTOR().cuda()
    hazards, survival, Y_hat, A_raw, m = model(h,h1,h2)
    print(hazards)